...
export const msgSend = (msg: string) => {
...
let constraints = { audio: { channels: 2, autoGainControl: false, echoCancellation: false, noiseSuppression: false }, video: true };
...
pc.onnegotiationneeded = async () => {
  try {
    ...
    websocketClient.emit("newDescription", { id: roomID, description: pc.localDescription });
  } catch (err) {
    console.error(err);
  } finally {
    makingOffer = false;
...
pc.onicecandidate = ({ candidate }) => websocketClient.emit("newDescription", { id: roomID, candidate: candidate });
